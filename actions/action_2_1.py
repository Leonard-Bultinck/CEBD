...
class AppFct2_1(QDialog):
    ...
    def refreshAllTables(self):
        self.refreshTable(self.ui.label, self.ui.table,
                          ...
                          "FROM (SELECT noSpec, nomSpec, dateRep "
                          "FROM LesRepresentations join LesSpectacles using(noSpec) "
                          "EXCEPT SELECT noSpec, nomSpec, dateRep FROM LesRepresentations "
                          "join LesSpectacles using (noSpec) join LesVentes using(dateRep))")
